...
public class $0122_Best_Time_To_Buy_and_Sell_Stock_II {
	...
	public int maxProfit(int[] prices) {
		...
		for (int i = 0; i < len - 1; ++i) {
			//	The price will fall! Sell the stock with current price now
			if (prices[i+1] < prices[i]) {
				profit += Math.max(0, prices[i] - lowpoint);
				lowpoint = prices[i+1];
			}
		}
		
		//	If at the end, it is still rising pattern, we shall sell it at the end of day
...
int prev_hold = Integer.MIN_VALUE / 2, prev_sold = 0;
		
		for (int i: prices) {
			int curr_hold = Math.max( prev_hold, prev_sold - i);
			int curr_sold = Math.max( prev_sold, prev_hold + i);
			prev_hold = curr_hold;
			prev_sold = curr_sold;
		}
		return prev_sold;
