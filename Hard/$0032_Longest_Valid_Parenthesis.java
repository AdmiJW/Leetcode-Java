...
public class $0032_Longest_Valid_Parenthesis {
	...
	public int longestValidParentheses2(String s) {
		...
		for (int i = 0; i < s.length(); ++i) {
			...
			if (c == '(') stack.push(i);	//	Record index of the opening parentheses
				...
				if (stack.isEmpty() ) stack.push(i);		//	If we've consume the 'base case' where there's actually no closing
															//	bracket, clear the state. Push current index as the base 
				else res = Math.max(res, i - stack.peek() );//	Otherwise, the top of stack now is the index of opening bracket
	...
	public int longestValidParentheses3(String s) {
		...
		for (char c: s.toCharArray() ) {
			...
			if (opening == closing) res = Math.max(res, opening+closing);
