...
public class $0927_Three_Equal_Parts {
	public int[] threeEqualParts(int[] arr) {
		...
		if (count1s % 3 != 0) return new int[] {-1, -1};
		if (count1s == 0) return new int[] {0,2};		// All 0, simply form the range
		count1s /= 3;
		...
		for (int pointer = range3+1; pointer < len; ++pointer, ++range1)		// Now we check, both pointer shall always be equal to have three equal binaries
			if (arr[pointer] != arr[range1]) return new int[] {-1, -1};			// Mismatch
		...
		for (int pointer = range3+1; pointer < len; ++pointer, ++range2)		// Now we check, both pointer shall always be equal to have three equal binaries
				if (arr[pointer] != arr[range2]) return new int[] {-1, -1};		// Mismatch
		
		return new int[] { range1 -1, range2 };
    }
}
