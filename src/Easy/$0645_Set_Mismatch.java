...
public class $0645_Set_Mismatch {
	...
	public int[] findErrorNums2(int[] nums) {
		...
		res[1] = nums[nums.length - 1] != nums.length? nums.length: res[1];	//	Handle edge case where missing is
																			//	nums.length
		return res;
	}
	...
	public int[] findErrorNums3(int[] nums) {
		...
		for (int i: nums) {
			i = Math.abs(i);					//	May be previously marked already, so make it positive
