...
public class $0696_Count_Binary_Substrings {
	...
	public int countBinarySubstrings(String s) {
		...
		for (int i = 1; i < len; ++i) {
			if (s.charAt(i) != s.charAt(i-1) ) {
				int l = i-1, r = i;
				while (l >= 0 && r < len && s.charAt(l) == s.charAt(i-1) && s.charAt(r) == s.charAt(i) ) {
					--l; ++r;
					++res;
				}
			}
	...
	public int countBinarySubstrings2(String s) {
		...
		for (int i = 0; i < len; ++i) {
			if (s.charAt(right) != s.charAt(i) ) {
				if (left != -1) 
					res += Math.min(right - left, i - right);
				left = right;
				right = i;
			}
		}
		...
		if (left != -1) 
			res += Math.min(len - right, right - left);
		return res;
	}
