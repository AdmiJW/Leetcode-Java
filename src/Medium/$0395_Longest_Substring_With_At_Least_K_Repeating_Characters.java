...
public class $0395_Longest_Substring_With_At_Least_K_Repeating_Characters {
	...
	public int longestSubstring(String s, int k) {
		...
		for (int st = 0; st < len; st ++ ) {
			...
			for (int end = st; end < len; end ++ ) {
				...
				if (unsolved == 0) res = Math.max(res, end - st);
			}
		}
		return res;
	}
	...
	public int longestSubstring2(String s, int k) {
		...
		for (int i = 0; i < len; i ++ ) {
			...
			if (freq[c] < k) {
				return Math.max( 
					longestSubstring2( s.substring(0, i-1), k ),
					longestSubstring2( s.substring(i + 1, len - 1), k )
				);
			}
		}
		return len;
