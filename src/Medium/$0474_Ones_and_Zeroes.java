...
public class $0474_Ones_and_Zeroes {

	public int findMaxForm(String[] strs, int m, int n) {
		final int rows = strs.length;
		final int zerodim = m+1;			//	Included 0. That's why m+1 and n+1
		final int onedim = n+1;
		
		int[][][] dp = new int[rows+1][zerodim][onedim];
		...
		for (int x = 1; x <= rows; ++x) {
			...
			for (int y = 0; y < zerodim; ++y) {
				...
				for (int z = 0; z < onedim; ++z) {
					...
					dp[x][y][z] = Math.max( dp[x][y][z], dp[x-1][y][z]);
				}
			}
		}
		return dp[rows][m][n];
	...
	//	Since we only need last row's result, reduce space complexity from ( len(strs) * m * n) to (m*n) only
	//	Note that in this form, we have to reverse iteration because we won't want to include the same bitstring twice!
	public int findMaxForm2(String[] strs, int m, int n) {
		final int rows = strs.length;
		final int zerodim = m+1;			//	Included 0. That's why m+1 and n+1
		final int onedim = n+1;
		
		int[][] dp = new int[zerodim][onedim];
		...
		for (int x = 1; x <= rows; ++x) {
			...
			for (int y = zerodim-1; y >= 0; --y) {
				...
				for (int z = onedim-1; z >= 0; --z) {
					//	We can include current string here. Compare with same column, prev row
					if (y - zero >= 0 && z - one >= 0) 
