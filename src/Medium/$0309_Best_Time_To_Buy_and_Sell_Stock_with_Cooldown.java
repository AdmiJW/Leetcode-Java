...
public class $0309_Best_Time_To_Buy_and_Sell_Stock_with_Cooldown {
	
	// DP solution - O(N^2) time, O(N) space
	public int maxProfit(int[] prices) {
		final int len = prices.length;
		int[] sellDP = new int[len+1];
		int[] readyDP = new int[len+1];
		...
		for (int i = 0; i < len; ++i) {
			// Cooldown
			readyDP[i+1] = Math.max(readyDP[i], sellDP[i]);
        ...
        return Math.max(readyDP[len], sellDP[len]);
    }
	...
	public int maxProfit2(int[] prices) {
		final int len = prices.length;
		int[] holdDP = new int[len + 1];
		int[] emptyDP = new int[len + 1];
		...
		for (int i = 1; i < len; ++i) {
			...
			holdDP[i+1] = Math.max(holdDP[i], emptyDP[i-1] - prices[i]);
			// For empty, we sell at current day or inherit from previous day
			emptyDP[i+1] = Math.max(emptyDP[i], holdDP[i] + prices[i]);
		}
        return emptyDP[len];
	}
}
