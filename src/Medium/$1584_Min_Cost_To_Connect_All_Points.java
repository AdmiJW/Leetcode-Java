...
public class $1584_Min_Cost_To_Connect_All_Points {
	...
	public int minCostConnectPoints(int[][] points) {
		...
		for (int i = 1; i < len; ++i) weights[i] = Integer.MAX_VALUE;
		...
		while (nodesIncluded < len) {
			...
			for (int i = 0; i < len; ++i) {
				if (visited[i]) continue;
				int cost = Math.abs(points[minNode][0] - points[i][0]) + Math.abs(points[minNode][1] - points[i][1]);
