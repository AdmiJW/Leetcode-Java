...
public class $0880_Decoded_String_At_Index {
	
    public String decodeAtIndex(String S, long K) {
    	Deque<Long> storeLength = new ArrayDeque<>();
    	Deque<Integer> storeMulti = new ArrayDeque<>();
    	
    	long currLen = 0;
    	
    	for (char c: S.toCharArray() ) {
    		if (Character.isAlphabetic(c) ) {
    			if (++currLen == K) return Character.toString(c);
    		}
    		else {
    			storeLength.push( currLen );
    			storeMulti.push(c - '0');
    			...
    			if (currLen >= K ) {
    				while ( !storeLength.isEmpty() && K <= storeLength.peek() * storeMulti.pop() ) {
    					long index = (K - 1) % storeLength.pop();
    					K = index + 1;
    				}
