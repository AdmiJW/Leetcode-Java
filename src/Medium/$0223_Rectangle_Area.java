...
public class $0223_Rectangle_Area {
    ...
    public int computeArea(int ax1, int ay1, int ax2, int ay2, int bx1, int by1, int bx2, int by2) {
        ...
        if (bx1 >= ax1 && bx1 <= ax2) {
            xIntersection = Math.min(ax2, bx2) - bx1;
        } 
        else if (ax1 >= bx1 && ax1 <= bx2) {
            xIntersection = Math.min(ax2, bx2) - ax1;
        }

        if (by1 >= ay1 && by1 <= ay2) {
            yIntersection = Math.min(ay2, by2) - by1;
        } 
        else if (ay1 >= by1 && ay1 <= by2) {
            yIntersection = Math.min(ay2, by2) - ay1;
        }

        int area1 = (ax2 - ax1) * (ay2 - ay1);
        int area2 = (bx2 - bx1) * (by2 - by1);
        // System.out.println(area1 + " " + area2 + " " + xIntersection + " " + yIntersection);
