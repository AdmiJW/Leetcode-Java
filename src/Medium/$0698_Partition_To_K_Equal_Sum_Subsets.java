...
public class $0698_Partition_To_K_Equal_Sum_Subsets {
	...
	public boolean canPartitionKSubsets(int[] nums, int k) {
        ...
        // Backtracking
        return backtrack(0, k, nums.length - 1, new boolean[nums.length], nums, sum / k);
    }
	
	// Returns true if there is a case found.
	private boolean backtrack(int setSum, int setLeft, int rightBound, boolean[] visited, int[] nums, int target) {
		// Base case - Already reached target! return true
		if (setLeft == 0) return true;
		
		// Set fulfilled. Proceed to next set
		if (setSum == target) 
			return backtrack(0, setLeft - 1, nums.length - 1, visited, nums, target);
	...
	public boolean canPartitionKSubsets2(int[] nums, int k) {
        ...
        Arrays.fill(dp, Integer.MIN_VALUE);     // - MIN_VALUE means invalid state
        ...
        for (int state = 0; state < (1 << len); ++state) {
        	...
        	if (dp[state] == Integer.MIN_VALUE) continue;
