...
public class $0421_Maximum_XOR_Of_Two_Numbers_In_An_Array {
	
	private static int mask = (int)Math.pow(2, 30);
	...
	public static int findMaximumXOR(int[] nums) {
		...
		constructTrie(nums[0], head);
		
		for (int i = 1; i < len; i ++ ) {
			res = Math.max( findMaxXOR( nums[i], head), res );
			constructTrie(nums[i], head);
		}
		return res;
	...
	private static int findMaxXOR(int n, TrieNode node) {
		int res = 0;
	...
	private static void constructTrie(int n, TrieNode node) {
		for (int i = mask; i > 0; i >>= 1) {
			if ( (n & i) != 0 ) {
				if (node.one == null) node.one = new TrieNode();
				node = node.one;
			} else {
