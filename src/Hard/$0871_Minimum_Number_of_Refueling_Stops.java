...
public class $0871_Minimum_Number_of_Refueling_Stops {
        ...
        for (int stationIdx = 0; stationIdx < len; ++stationIdx) {
        	for (int stops = stationIdx+1; stops > 0; --stops) {
        		...
        		if ( stations[stationIdx][0] <= dp[stops-1] )
        			dp[stops] = Math.max(dp[stops], dp[stops-1] + (long)stations[stationIdx][1] );
        	}
        }
	...
	public int minRefuelStops2(int target, int startFuel, int[][] stations) {
		...
		while (!pq.isEmpty() && tank < target - laststop) {
			++stops;
			tank += pq.poll();
		}
