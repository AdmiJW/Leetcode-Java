...
public class $0132_Palindrome_Partitioning_II {
	...
	public int minCut(String s) {
        ...
        for (int right = 0; right < len; ++right) {
        	...
        	for (int left = 0; left <= right; ++left) {
        		...
        		if (s.charAt(left) == s.charAt(right) && (left+1 > right-1 || isPalindrome[left+1][right-1])) {
        			isPalindrome[left][right] = true;
        			minCuts = (left == 0)? 0: Math.min(minCuts, dp[left-1] + 1);
        		}
        	}
    		dp[right] = minCuts;
        }
