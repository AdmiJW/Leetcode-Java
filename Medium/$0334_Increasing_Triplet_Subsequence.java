...
public class $0334_Increasing_Triplet_Subsequence {

	//	O(2N) == O(N) time, O(N) space solution
	public boolean increasingTriplet(int[] nums) {
		final int len = nums.length;
		int[] maxElem = new int[len];
		maxElem[len - 1] = nums[len - 1];
		
		for (int i = len - 2; i >= 0; --i)
			maxElem[i] = Math.max( maxElem[i+1], nums[i] );
		
		int min = nums[0];
		for (int pt = 1; pt < len - 1; ++pt) {
			if (min < nums[pt] && maxElem[pt+1] > nums[pt] ) return true;
			min = Math.min(nums[pt], min);
		}
		return false;
	}
	...
	public boolean increasingTriplet2(int[] nums) {
		...
		while (curridx < len - 1) {
			if (min < nums[curridx] ) {
				...
				if (localidx >= len) return false;	//	No more increasing subsequence after this? Impossible then
