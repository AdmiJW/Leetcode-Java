...
public class $0289_Game_of_Life {
	...
	public void gameOfLife2(int[][] board) {
		...
		for (int i = 0; i < row; ++i ) {
			for (int j = 0; j < col; ++j) {
				int s = findSumNeighbor( board, i , j) + 1;		//	Offset by 1 to prevent ambiguous 0
				board[i][j] = (board[i][j] == 0? -1: 1) * s;
			}
		}
		...
		for (int i = 0; i < row; ++i ) {
			for (int j = 0; j < col; ++j) {
				if (board[i][j] < 0) 
					board[i][j] = -board[i][j] - 1 == 3? 1: 0;
				else 
					board[i][j] = board[i][j] - 1 == 2 || board[i][j] - 1 == 3? 1: 0; 
			}
		}
	}
	...
	private int findSumNeighbor( int[][] board, int r, int c ) {
		final int row = board.length;
		final int col = board[0].length;
	...
	private int findSumNeighborUnbounded( int[][] board, int r, int c ) {
		final int row = board.length;
		final int col = board[0].length;
