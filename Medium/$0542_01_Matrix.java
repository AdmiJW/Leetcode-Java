...
public class $0542_01_Matrix {
	...
	public int[][] updateMatrix(int[][] mat) {
		...
		for (int i = 0; i < r; ++i) {
			for (int j = 0; j < c; ++j) {
				if (mat[i][j] == 0)
					...
					if (i > 0) res[i][j] = Math.min(res[i][j], res[i-1][j] + 1);	// Check above
					if (j > 0) res[i][j] = Math.min(res[i][j], res[i][j-1] + 1);	// Check left
				}
			}
		}
		...
		for (int i = r-1; i >= 0; --i) {
			for (int j = c-1; j >= 0; --j) {
				if (i+1 < r) res[i][j] = Math.min(res[i][j], res[i+1][j] + 1);	// Check below
				if (j+1 < c) res[i][j] = Math.min(res[i][j], res[i][j+1] + 1);	// Check right
			}
		}
